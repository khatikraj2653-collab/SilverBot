...
from graph.nodes import generate_prediction, FACTOR_WEIGHTS, INDUSTRIAL_FACTORS, MONETARY_FACTORS

fred = Fred(api_key=os.getenv("FRED_API_KEY"))

# ---------- Reused from GoldBot Backtest 1 for cross-bot comparability ----------
TEST_DATES = [
    datetime(2025,9,5), datetime(2025,9,16), datetime(2025,10,3), datetime(2025,10,17),
    datetime(2025,11,5), datetime(2025,11,20), datetime(2025,12,8), datetime(2025,12,16),
    datetime(2026,1,2), datetime(2026,1,15), datetime(2026,2,2), datetime(2026,2,17),
    datetime(2026,3,2), datetime(2026,3,17), datetime(2026,4,6), datetime(2026,4,20),
    datetime(2026,5,4), datetime(2026,5,20), datetime(2026,6,5), datetime(2026,6,15),
]
...
# ---------- MA50 point-in-time ground truth (silver, gold, copper) ----------
...
def classify_regime_vs_proxy(target_price, target_ma50, proxy_price, proxy_ma50):
    """ALIGNED if target and proxy trend the same direction relative to their own MA50,
    DIVERGENT if opposite. Used separately for silver-vs-gold (monetary) and silver-vs-copper (industrial)."""
    ...
    return "ALIGNED" if target_above == proxy_above else "DIVERGENT", target_above, proxy_above
...
def build_historical_state(as_of_date):
    ...
    state = {
        ...
        # Tier 2 - forced neutral for now (same rationale as GoldBot)
        "mining_supply_growth": "Mining Supply Growth: Data unavailable (backtest - no historical text recovery)",
        "solar_panel_demand": "Solar Panel Demand: Data unavailable (backtest - no historical text recovery)",
        "geopolitical_risk": "Geopolitical Risk: Data unavailable (backtest - no historical text recovery)",
    }
...
def run_backtest():
    ...
    for i, as_of_date in enumerate(TEST_DATES):
        ...
        if silver_price is not None and silver_ma50 is not None:
            ...
            combined_correct = (combined_status == "ON" and silver_above) or (combined_status == "OFF" and not silver_above)

        monetary_regime, silver_above_m, gold_above_m = (None, None, None)
        if None not in (silver_price, silver_ma50, gold_price_gt, gold_ma50_gt):
            res = classify_regime_vs_proxy(silver_price, silver_ma50, gold_price_gt, gold_ma50_gt)
            monetary_regime, silver_above_m, gold_above_m = res

        industrial_regime, silver_above_i, copper_above_i = (None, None, None)
        if None not in (silver_price, silver_ma50, copper_price, copper_ma50):
            res = classify_regime_vs_proxy(silver_price, silver_ma50, copper_price, copper_ma50)
            industrial_regime, silver_above_i, copper_above_i = res

        monetary_correct = None
        if monetary_regime in ("ALIGNED", "DIVERGENT"):
            gt_monetary_bullish = gold_above_m
            monetary_correct = (monetary_status == "ON") == gt_monetary_bullish

        industrial_correct = None
        if industrial_regime in ("ALIGNED", "DIVERGENT"):
            gt_industrial_bullish = copper_above_i
            industrial_correct = (industrial_status == "ON") == gt_industrial_bullish
    ...
    df.to_csv("backtest1_results.csv", index=False)
    ...
    print("\nSaved to backtest1_results.csv")
